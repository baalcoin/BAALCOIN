...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in BAAL/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in BAAL/kb\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"BAALaddress\"\n"
            "\nReturn information about the given BAAL address.\n"
            "\nArguments:\n"
            "1. \"BAALaddress\"     (string, required) The BAAL address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"BAALaddress\", (string) The BAAL address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BAAL address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are BAAL addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) BAAL address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"BAALaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"BAALaddress\"  (string, required) The BAAL address to use for the signature.\n"
